...
#
# 2025/12/18 ImageMaskDatasetGenerator.py
...
class ImageMaskDatasetGenerator:
  ...
  def __init__(self, 
    ...
    self.image_files = glob.glob(self.images_dir + "/*.nii")
    self.image_files = sorted(self.image_files)

    self.mask_files = glob.glob(self.masks_dir + "/*.nii")
  ...
  def generate_mask_files(self, niigz_file, index ):
    ...
    for i in range(d):
      ...
      if img.any() >0:
        ...
        cv2.imwrite(filepath, img)
        if self.augmentation:
           self.augment(img, filename, self.output_masks_dir, border=(0, 0, 0), mask=False)
...
if __name__ == "__main__":
  try:
    ...
    images_dir  = "./Left_Atrial_Segmentation/imagesTr/"
    masks_dir   = "./Left_Atrial_Segmentation/labelsTr/"
